...
plot(boost2, i="LoanAmount_Xform")
plot(boost2, i="Income_Xform")
```
Recall we applied transforamtions on these features.  By reversing the transforms, we find that the first plot shows `LoanAmount` is negatively correlated with the response `Loan_Status` before `r exp(5.7)`K. The second plot indicate `Income` is positively correlated with `Loan_Status` when it less than $20,000.
